...
class OptimizeRequest(BaseModel):
    ...
    lint_loss_pct: float = 0      # applies to the whole mixing process, not per cotton
    comber_noil_pct: float = 0    # applies to the whole mixing process, not per cotton
    cottons: list[Cotton]
...
def optimize(req: OptimizeRequest):
    ...
    # waste % = trash (per cotton) + lint loss + comber noil (whole process), as per spec sheet
    waste_pct = [c.trash_pct + req.lint_loss_pct + req.comber_noil_pct for c in cottons]
    ...
    for i, c in enumerate(cottons):
        ...
        rows.append({
            ...
            "trash_plus_lint": round(c.trash_pct + req.lint_loss_pct, 2),
